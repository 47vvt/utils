...
scoop install firefox vscode python spotify notion discord # apps
...
@(set-variable "0=%~f0"^)#) & powershell -nop -c iex([io.file]::ReadAllText($env:0)) & exit/b
...
if (test-path "$IFEO\0") {
  ...
  remove-item $file -force -ea 0 >''; remove-item $CREDGE -rec -force -ea 0 >'';
  ...
  [io.file]::WriteAllText($file, $vbs) >''; start-process -nonew cmd "/d/x/r mklink /J ""$CREDGE"" ""$EDGE"" >nul"
